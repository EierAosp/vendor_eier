...
func init() {
	android.RegisterModuleType("eier_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = eierExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = eierExpandVariables(ctx, dep_file)
	...
	cmd := eierExpandVariables(ctx, String(g.properties.Cmd))
